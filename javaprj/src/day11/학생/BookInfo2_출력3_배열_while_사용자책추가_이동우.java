...
public class BookInfo2_출력3_배열_while_사용자책추가_이동우 {
	...
	public static void main(String[] args) {
		...
		//등록한 책의 정보를 저장하는 역할
		ArrayList<BookInfo2_이동우> bk = new ArrayList<>();
		
		
		//첫번째 책 배열 (여행)
		BookInfo2_이동우[] book = new BookInfo2_이동우[3];
		
		book[0] = new BookInfo2_이동우("리얼 프라하 2025~2026", "안지선", 18900);
		book[1] = new BookInfo2_이동우("에이든 우리나라 전국 여행지도", "타블라라사, 이정기", 17820);
		book[2] = new BookInfo2_이동우("디스 이즈 도쿄 THIS IS TOKYO", "박설희", 17820);
	
	
		//두번째 책 배열 (베스트셀러)
		BookInfo2_이동우[] book2 = new BookInfo2_이동우[3];
		book2[0] = new BookInfo2_이동우("국민이 먼저입니다", "한동훈", 19800);
		book2[1] = new BookInfo2_이동우("초역 부처의 말", "코이케 류노스케", 16020);
		book2[2] = new BookInfo2_이동우("환율의 대전환", "오건영", 24300);
		
		
		//사용자 입력 책 배열
		BookInfo2_이동우[] book3 = new BookInfo2_이동우[3];
		int index = 0;
...
//				System.out.println(bookSelect);
				...
				for(int i = 0; i < book.length; i++) {
					...
					Select2--;
					BookInfo2_이동우 selectBook = book[Select2];
					...
					Select3--;
					BookInfo2_이동우 selectBook = book2[Select3];
			...
			case 3:
				...
				BookInfo2_이동우 b = new BookInfo2_이동우(title, author, price);
				book3[index] = b;
				index++;
