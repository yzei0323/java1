...
public class 권지언_Managebook {

	static int idx = 0;
	static 권지언_Book[] arr = new 권지언_Book[0];
	...
	public static 권지언_Book[] createBook(권지언_Book[] arr, String title, String author, int price) {
		권지언_Book newBook = new 권지언_Book(title, author, price);
		권지언_Book[] newBooks = new 권지언_Book[arr.length + 1];
	...
	public static 권지언_Book[] deleteBook(권지언_Book[] arr, int deleteIndex) {
	...
	public static void updateBook(권지언_Book[] arr, int updateIndex, Scanner sc) {
		if (updateIndex < 0 || updateIndex >= arr.length) {
			...
			if (input.equals("제목")) {
				...
				권지언_Book search = arr[updateIndex];
